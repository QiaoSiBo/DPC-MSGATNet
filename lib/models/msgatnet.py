...
class medt_net(nn.Module):
    ...
    def __init__(self, block, block_2, layers, num_classes=2, zero_init_residual=True,
                 groups=8, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, s=0.125, img_size=128, block_size=32, imgchan = 3):
        ...
        self.layer1_p = self._make_layer(block_2, int(128 * s), layers[0])
        self.layer2_p = self._make_layer(block_2, int(256 * s), layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0])
        self.layer3_p = self._make_layer(block_2, int(512 * s), layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1])
        self.layer4_p = self._make_layer(block_2, int(1024 * s), layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2])
        ...
        self.atten_block = Attention_blcok(int(128*s), int(128*s*s))
    ...
    def _forward_impl(self, x):
        ...
        num_blocks = self.img_size // self.block_size
        x_loc = x.clone()
        #start
        for i in range(0, num_blocks):
            for j in range(0, num_blocks):
                ...
                #print("x_p", x_p.size())   ###[1, 3, 32, 32]
                # begin patch wise
                x_p = self.conv1_p(x_p)
                x_p = self.bn1_p(x_p)
                # x = F.max_pool2d(x,2,2)
                x_p = self.relu(x_p)
                #print("x_p", x_p.size())   ###[1, 64, 16, 16]

                x_p = self.conv2_p(x_p)
                x_p = self.bn2_p(x_p)
                # x = F.max_pool2d(x,2,2)
                x_p = self.relu(x_p)
                #print("x_p", x_p.size())   ###[1, 128, 16, 16]
...
class Attention_blcok(nn.Module):

    def __init__(self, c_in, c_out):
        super(Attention_blcok, self).__init__()
        self.w_g = nn.Sequential(
            nn.Conv2d(c_in, c_out, kernel_size=1, stride=1, padding=0, bias=True),
        ...
        self.psi = nn.Sequential(
            nn.Conv2d(c_out, 1, kernel_size=1, stride=1, padding=0, bias=True),
            nn.BatchNorm2d(1),
            nn.Sigmoid()
        )
        self.relu = nn.ReLU(inplace=True)
    ...
    def forward(self, g_f, l_f):
        g1 = self.w_g(g_f)
        l1 = self.w_l(l_f)
        psi_relu = self.relu(g1+l1)
        psi = self.psi(psi_relu)
        fusion = (g_f + l_f) * psi
        return fusion

class msgan_net(nn.Module):

    def __init__(self, block, block_2, layers, num_classes=5, groups=8,
                 width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, s=0.125, img_size=128, block_size=32, imgchan=3):
        super(msgan_net, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        self.img_size = img_size
        ...
        # Decoder
        self.decoder1 = nn.Conv2d(int(512*s),  int(256*s), kernel_size=3, stride=1, padding=1)
        self.decoder2 = nn.Conv2d(int(256*s), int(128*s), kernel_size=3, stride=1, padding=1)

        img_chan = int(128*s)
        ###Encoder2
        self.conv4 = nn.Conv2d(img_chan, self.inplanes, kernel_size=7, stride=2, padding=3, bias=False)
        self.conv5 = nn.Conv2d(self.inplanes, 128, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv6 = nn.Conv2d(128, self.inplanes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn4 = norm_layer(self.inplanes)
        self.bn5 = norm_layer(128)
        self.bn6 = norm_layer(self.inplanes)
        self.relu = nn.ReLU(inplace=True)
        self.layer3 = self._make_layer(block, int(128 * s), layers[0], kernel_size=(img_size//2))
        self.layer4 = self._make_layer(block, int(256 * s), layers[1], stride=2, kernel_size=(img_size//2),
                                   dilate=replace_stride_with_dilation[0])

        # Decoder2
        self.decoder3 = nn.Conv2d(int(512*s),  int(256*s), kernel_size=3, stride=1, padding=1)
        ...
        # Decoder
        self.decoder1_p = nn.Conv2d(int(512*s),  int(256*s), kernel_size=3, stride=1, padding=1)
        self.decoder2_p = nn.Conv2d(int(256*s), int(128*s), kernel_size=3, stride=1, padding=1)

        ###local_2
        imgchan = int(128*s)
        self.conv4_p = nn.Conv2d(imgchan, self.inplanes, kernel_size=7, stride=2, padding=3, bias=False)
        self.conv5_p = nn.Conv2d(self.inplanes, 128, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv6_p = nn.Conv2d(128, self.inplanes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn4_p = norm_layer(self.inplanes)
        self.bn5_p = norm_layer(128)
        self.bn6_p = norm_layer(self.inplanes)

        self.layer3_p = self._make_layer(block_2, int(128 * s), layers[0])
        self.layer4_p = self._make_layer(block_2, int(256 * s), layers[1], stride=2,
                                         dilate=replace_stride_with_dilation[0])
        ...
        self.soft     = nn.Softmax(dim=1)
        self.atten_block = Attention_blcok(int(128*s), int(128*s*s))
        #self.local_atten = Attention_blcok(16, 8)
    ...
    def _forward_impl(self, x):
        ...
        xin = x.clone()
        x = self.conv1(x)
        #print('x:', x.shape)   ###(1, 32, 112, 112)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.conv2(x)
        #print("x:", x.shape)   ###(1, 128, 112, 112)
        x = self.bn2(x)
        x = self.relu(x)
        x = self.conv3(x)
        #print("x:", x.shape)   ### (1, 32, 112, 112)
        ...
        x = self.relu(x)
        x1 = self.layer1(x)
        #print("x1", x1.shape)   ###(1, 128, 112, 112)
        x2 = self.layer2(x1)
        #print("x2", x2.shape)   ###(1, 256, 56, 56)

        x = F.relu(F.interpolate(self.decoder1(x2), scale_factor=(2, 2), mode='bilinear'))
        #print("x:", x.shape)   ###(1, 128, 112, 112)

        x = torch.add(x, x1)
        x_d0 = F.relu(F.interpolate(self.decoder2(x), scale_factor=(2, 2), mode='bilinear'))
        #print("x:", x_d0.shape)   ###(1, 64, 224, 224)
        # end of full image training
        num_blocks = self.img_size // self.block_size
        x_loc_1 = x_d0.clone()
        #start
        for i in range(0, num_blocks):
            for j in range(0, num_blocks):
                ...
                # x = F.max_pool2d(x,2,2)
                x_p = self.relu(x_p)
                #print("x_p", x_p.size())   ###[1, 256, 16, 16]
                ...
                # pdb.set_trace()
                x1_p = self.layer1_p(x_p)
                #print("x1_p", x1_p.size())  ###[1, 128, 16, 16]

                x2_p = self.layer2_p(x1_p)
                #print("x2_p", x2_p.size())  ###[1, 256, 8, 8]

                x_p = F.relu(F.interpolate(self.decoder1_p(x2_p), scale_factor=(2, 2), mode='bilinear'))
                x_p = torch.add(x_p, x1_p)
                #print("x_p", x_p.size())   ###[1, 128, 16, 16]
                x_p = F.relu(F.interpolate(self.decoder2_p(x_p), scale_factor=(2, 2), mode='bilinear'))
                #print("x_p", x_p.size())   ###[1, 64, 32, 32]

                x_loc_1[:, :, self.block_size*i:self.block_size*(i+1), self.block_size*j:self.block_size*(j+1)] = x_p
        #########the first fusion#######
        #fusion = self.atten_block(x_d0, x_loc_1)
        fusion = torch.add(x_d0, x_loc_1)

        ####U-2
        x = self.conv4(fusion)
        #print('x:', x.shape)   ###(1, 256, 112, 112)
        x = self.bn4(x)
        x = self.relu(x)
        x = self.conv5(x)
        #print("x:", x.shape)   ###(1, 128, 112, 112)
        ...
        x = torch.add(x, x1)
        x = F.relu(F.interpolate(self.decoder4(x), scale_factor=(2, 2), mode='bilinear'))
        #print("x:", x.shape)   ###(1, 64, 224, 224)

        x = torch.add(x, x_d0)

        # end of full image training
        x_loc_2 = x.clone()
        x_fusion = fusion.clone()
        #####The second local network#######
        for i in range(0, num_blocks):
            for j in range(0, num_blocks):

                x_p = x_fusion[:, :, self.block_size*i:self.block_size*(i+1), self.block_size*j:self.block_size*(j+1)]
                #print("x_p", x_p.size())   ###[1, 64, 32, 32]
                # begin patch wise
                x_p = self.conv4_p(x_p)
                x_p = self.bn4_p(x_p)
                # x = F.max_pool2d(x,2,2)
                x_p = self.relu(x_p)
                #print("x_p", x_p.size())   ###[1, 256, 16, 16]

                x_p = self.conv5_p(x_p)
                x_p = self.bn5_p(x_p)
                # x = F.max_pool2d(x,2,2)
                x_p = self.relu(x_p)
                #print("x_p", x_p.size())   ###[1, 128, 16, 16]
                x_p = self.conv6_p(x_p)
                x_p = self.bn6_p(x_p)
                # x = F.max_pool2d(x,2,2)
                x_p = self.relu(x_p)
                #print("x_p", x_p.size())   ###[1, 256, 16, 16]
                ...
                # x = self.maxpool(x)
                # pdb.set_trace()
                x1_p = self.layer3_p(x_p)
                #print("x1_p", x1_p.size())  ###[1, 128, 16, 16]

                x2_p = self.layer4_p(x1_p)
                #print("x2_p", x2_p.size())  ###[1, 256, 8, 8]
                ...
                x_loc_2[:, :, self.block_size*i:self.block_size*(i+1), self.block_size*j:self.block_size*(j+1)] = x_p

        x_loc_2 = x_loc_1 + x_loc_2

        #x = torch.add(x,x_loc)

        ####the second fusion
        #x = self.atten_block(x, x_loc_2)
        x = torch.add(x, x_loc_2)
...
def MSGAN(pretrained=False, **kwargs):
    model = msgan_net(AxialBlock_dynamic, AxialBlock_wopos, [1, 2, 4, 1], s=0.5,  **kwargs)
    return model
